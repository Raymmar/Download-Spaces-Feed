...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/webhook", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `${timestamp} - Webhook data validated successfully:`,
          validatedData
        );
      } catch (validationError) {
        console.error(
          `${timestamp} - Webhook validation failed:`,
          validationError
        );
      ...
      if (recentDuplicates.length > 0) {
        console.log(
          `${timestamp} - Duplicate webhook detected within 24 hours, skipping insertion`
        );
      ...
      try {
        ...
        console.log(
          `${timestamp} - Webhook stored successfully:`,
          insertedWebhook
        );
      } catch (dbError) {
  ...
  app.get("/api/events", (req, res) => {
    ...
    req.on("close", () => {
      clients.delete(res);
      console.log(
        "Client disconnected from SSE. Remaining clients:",
        clients.size
      );
    });
  });
